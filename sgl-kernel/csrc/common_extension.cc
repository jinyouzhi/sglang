...
TORCH_LIBRARY_FRAGMENT(sgl_kernel, m) {
  ...
  m.def("fast_topk(Tensor score, Tensor indices, Tensor lengths, int topk, Tensor? row_starts) -> ()");
  ...
  m.def(
      ...
      "cu_seqlens_q, int topk, Tensor? row_starts) -> ()");
  ...
  m.def(
      ...
      "topk_indices_offset, int topk, Tensor ? row_starts) -> ()");
