...
__device__ void naive_topk_cuda(const float* __restrict__ score, int32_t* __restrict__ indice, int32_t length, int32_t topk) {
...
__device__ void naive_topk_transform_ragged(
    const float* __restrict__ score, int32_t length, int32_t* __restrict__ topk_indices_ragged, int32_t offset, int32_t topk) {
  const auto tid = threadIdx.x;
  for (auto i = tid; i < topk; i += kThreadsPerBlock) {
...
__device__ void fast_topk_cuda_tl(const float* __restrict__ input, int* __restrict__ index, int row_start, int length, int topk) {
...
__global__ __launch_bounds__(kThreadsPerBlock)  // decode
  ...
  if (length <= topk) {
    ...
    int* s_indices = reinterpret_cast<int*>(smem_buf + kSmem);
...
__global__ __launch_bounds__(kThreadsPerBlock)  // prefill, ragged kv
  ...
  if (length <= topk) {
    ...
    int* s_indices = reinterpret_cast<int*>(smem_buf + kSmem);
    fast_topk_cuda_tl(score, s_indices, row_start, length, topk);
    // copy s_indices to dst with offset
    for (int i = tid; i < topk; i += kThreadsPerBlock) {
...
auto get_params(
  ...
  TORCH_CHECK(topk > 0, "topk must be > 0, got ", topk);
...
template <auto* f>
void setup_kernel_max_dynamic_smem(size_t required_smem) {
  static size_t configured_max = 0;
  if (required_smem <= configured_max) return;
#ifdef USE_ROCM
  auto result = ::cudaFuncSetAttribute(
      reinterpret_cast<const void*>(f), ::cudaFuncAttributeMaxDynamicSharedMemorySize, required_smem);
#else
  auto result = ::cudaFuncSetAttribute(f, ::cudaFuncAttributeMaxDynamicSharedMemorySize, required_smem);
#endif
  TORCH_CHECK(result == cudaSuccess, "setup_kernel_max_dynamic_smem failed: ", ::cudaGetErrorString(result));
  configured_max = required_smem;
}
...
void fast_topk_interface(
  ...
  CHECK_CUDA(lengths);
  const auto params = get_params(score, lengths, static_cast<int32_t>(topk), row_starts_opt, indices);
  const auto B = score.size(0);
...
void fast_topk_transform_interface(
  ...
  const auto params = get_params(score, lengths, static_cast<int32_t>(topk), row_starts_opt);
  const auto B = score.size(0);
  ...
  const size_t transform_smem = kSmem + static_cast<size_t>(topk) * sizeof(int);
  const auto is_decode = !row_starts_opt.has_value() && prefill_bs == B;
  if (is_decode) {
    setup_kernel_max_dynamic_smem<topk_transform_decode_kernel>(transform_smem);
    topk_transform_decode_kernel<<<grid, block, transform_smem, stream>>>(
        params, dst_page_table.data_ptr<int32_t>(), src_page_table.data_ptr<int32_t>(), src_stride);
  } else {
    setup_kernel_max_dynamic_smem<topk_transform_prefill_kernel>(transform_smem);
    topk_transform_prefill_kernel<<<grid, block, transform_smem, stream>>>(
        params,
...
void fast_topk_transform_ragged_interface(
  ...
  const size_t transform_smem = kSmem + static_cast<size_t>(topk) * sizeof(int);
  setup_kernel_max_dynamic_smem<topk_transform_prefill_ragged_kernel>(transform_smem);
  topk_transform_prefill_ragged_kernel<<<grid, block, transform_smem, stream>>>(
